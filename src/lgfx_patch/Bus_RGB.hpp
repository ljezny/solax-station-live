...
// Použít originální LovyanGFX includes
...
//----------------------------------------------------------------------------
  ...
  class Bus_RGB_Patched : public IBus
  {
  public:
    // VSYNC callback typ - voláno z ISR při každém VSYNC
    typedef void (*vsync_callback_t)(void* user_ctx);
    ...
    // Nastavit VSYNC callback - bude volán z ISR při každém VSYNC
    void setVSyncCallback(vsync_callback_t callback, void* user_ctx = nullptr) {
      _vsync_callback = callback;
      _vsync_user_ctx = user_ctx;
    }
  ...
  private:
    ...
    // VSYNC callback
    vsync_callback_t _vsync_callback = nullptr;
    void* _vsync_user_ctx = nullptr;
