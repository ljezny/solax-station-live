...
//----------------------------------------------------------------------------
  ...
  void Bus_RGB_Patched::config(const config_t& cfg)
  {
    _cfg = cfg;
  }


  IRAM_ATTR void Bus_RGB_Patched::lcd_default_isr_handler(void *args)
  {
    Bus_RGB_Patched *me = (Bus_RGB_Patched*)args;
    ...
    if (intr_status & LCD_LL_EVENT_VSYNC_END) {
      ...
      // PATCH: Volat user VSYNC callback
      if (me->_vsync_callback) {
        me->_vsync_callback(me->_vsync_user_ctx);
      }
    }
  }
  ...
  bool Bus_RGB_Patched::init(void)
  {
...
/*/
  ...
  uint8_t* Bus_RGB_Patched::getDMABuffer(uint32_t length)
  {
    return _frame_buffer;
    // return _rgb_panel->fb;
  }

  void Bus_RGB_Patched::release(void)
  {
    if (_intr_handle) {
